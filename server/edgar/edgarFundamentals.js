...
const EDGAR_REQUEST_TIMEOUT_MS = Number(process.env.EDGAR_REQUEST_TIMEOUT_MS) || 30_000;
...
async function limitedFetch(url, { parse = "json" } = {}) {
  ...
  while (attempt < OUTBOUND_MAX_RETRIES) {
    ...
    const timeoutId = setTimeout(() => controller.abort(), EDGAR_REQUEST_TIMEOUT_MS);
    try {
...
async function loadLocalDirectory() {
  const candidates = [
    path.join(DATA_DIR, "company_tickers_exchange.json"),
    path.join(DATA_DIR, "edgar", "company_tickers_exchange.json")
  ];
...
export function buildPeriodsFromCompanyfacts({
  ticker,
  cik,
  companyfacts,
  companyName = null,
  ...
  if (!c) throw new Error("cik is required");
  const factsObj = companyfacts || null;
  if (!factsObj || typeof factsObj !== "object") throw new Error("companyfacts is required");
  const name = companyName || factsObj?.entityName || null;
  return buildPeriods({
    ticker: t,
    cik: c,
    facts: factsObj,
