...
async function processSingleTicker(ticker, { createEvent = false } = {}) {
  try {
    ...
    const filing = await fetchLatestRelevantFiling(ticker);
    await processFilingForTicker(ticker, filing, { createEvent });
...
async function runIncrementalSweep() {
  ...
  for (const row of tickers) {
    extractState.current = row.ticker;
    const alreadyCheckedToday = row.lastCheckedAt?.slice(0, 10) === today;
    if (alreadyCheckedToday && hasFundamentalsCache(row.ticker)) continue;
    try {
      const latestFiling = await fetchLatestRelevantFiling(row.ticker);
      const hasNew = latestFiling?.filed && (!row.lastFilingDate || Date.parse(latestFiling.filed) > Date.parse(row.lastFilingDate));
      if (hasNew) {
        await processFilingForTicker(row.ticker, latestFiling, { createEvent: true });
        processedAny = true;
        extractState.lastHttpStatus = 200;
      } else {
